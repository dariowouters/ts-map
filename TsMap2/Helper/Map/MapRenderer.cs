...
namespace TsMap2.Helper.Map {
    public class MapRenderer {
        ...
        private readonly int[] zoomCaps = { 1000, 5000, 18500, 45000 };
        ...
        public void Render( Graphics g, Rectangle clip, float scale, PointF startPoint, MapPalette palette, RenderFlags renderFlags = RenderFlags.All ) {
            ...
            int zoomIndex = ScsRenderHelper.GetZoomIndex( clip, scale );
            ...
            // Log.Debug( "Flag ferry {0}", palette.Road );

            if ( renderFlags.IsActive( RenderFlags.FerryConnections ) ) {
                List< TsMapFerryItem > ferryConnections = _store.Map.FerryConnections.Where( item => !item.Hidden )
                                                                .ToList();
            ...
            long ferryTime = DateTime.Now.Ticks - ferryStartTime;

            long mapAreaStartTime = DateTime.Now.Ticks;
            if ( renderFlags.IsActive( RenderFlags.MapAreas ) ) {
                List< TsMapAreaItem > mapAreas = _store.Map.MapAreas.Where( item =>
            ...
            long mapAreaTime = DateTime.Now.Ticks - mapAreaStartTime;

            long prefabStartTime = DateTime.Now.Ticks;
            List< TsMapPrefabItem > prefabs = _store.Map.Prefabs.Where( item =>
                                                                            item.X    >= startPoint.X - itemDrawMargin
                                                                            && item.X <= endPoint.X   + itemDrawMargin
                                                                            && item.Z >= startPoint.Y - itemDrawMargin
                                                                            && item.Z <= endPoint.Y   + itemDrawMargin
            ...
            long prefabTime = DateTime.Now.Ticks - prefabStartTime;

            long roadStartTime = DateTime.Now.Ticks;
            if ( renderFlags.IsActive( RenderFlags.Roads ) ) {
                List< TsMapRoadItem > roads = _store.Map.Roads.Where( item =>
                                                                          item.X    >= startPoint.X - itemDrawMargin
                                                                          && item.X <= endPoint.X   + itemDrawMargin
                                                                          && item.Z >= startPoint.Y - itemDrawMargin
                                                                          && item.Z <= endPoint.Y   + itemDrawMargin
            ...
            long roadTime = DateTime.Now.Ticks - roadStartTime;

            long mapOverlayStartTime = DateTime.Now.Ticks;
            if ( renderFlags.IsActive( RenderFlags.MapOverlays ) ) {
                List< TsMapMapOverlayItem > overlays = _store.Map.MapOverlays.Where( item =>
                                                                                         item.X    >= startPoint.X - itemDrawMargin
                                                                                         && item.X <= endPoint.X   + itemDrawMargin
                                                                                         && item.Z >= startPoint.Y - itemDrawMargin
                                                                                         && item.Z <= endPoint.Y   + itemDrawMargin
            ...
            long mapOverlayTime = DateTime.Now.Ticks - mapOverlayStartTime;

            long mapOverlay2StartTime = DateTime.Now.Ticks;
            if ( renderFlags.IsActive( RenderFlags.MapOverlays ) ) {
                List< TsMapCompanyItem > companies = _store.Map.Companies.Where( item =>
                                                                                     item.X    >= startPoint.X - itemDrawMargin
                                                                                     && item.X <= endPoint.X   + itemDrawMargin
                                                                                     && item.Z >= startPoint.Y - itemDrawMargin
                                                                                     && item.Z <= endPoint.Y   + itemDrawMargin
            ...
            long mapOverlay2Time = DateTime.Now.Ticks - mapOverlay2StartTime;

            long cityStartTime = DateTime.Now.Ticks;
            if ( renderFlags.IsActive( RenderFlags.CityNames ) ) // TODO: Fix position and scaling
            {
                List< TsMapCityItem > cities = _store.Map.Cities.Where( item => !item.Hidden ).ToList();
            ...
            long cityTime = DateTime.Now.Ticks - cityStartTime;
