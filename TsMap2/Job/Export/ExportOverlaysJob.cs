...
namespace TsMap2.Job.Export {
    public class ExportOverlaysJob : ThreadJob {
        protected override void Do() {
            ...
            Log.Debug( "[Job][ExportOverlays] Companies : {0}", Store().Map.Overlays.Company.Count );
            Log.Debug( "[Job][ExportOverlays] Parking : {0}",   Store().Map.Overlays.Parking.Count );

            // Fixme: Overlays wasn't exported properly. The process seem to be stopped before the end of writing on file.
            var mapFactory = new TsOverlaysJsonFactory( Store().Map.Overlays );
            mapFactory.Save();
