...
namespace TsMap2.Model.TsMapItem {
    public class TsCutsceneItem : TsItem {
        public TsCutsceneItem( TsSector sector, int startOffset ) : base( sector, startOffset ) {
            ...
            if ( this.Sector.Version >= 884 )
                this.TsCutsceneItem844( startOffset );
            else
                Log.Warning(
        ...
        public void TsCutsceneItem844( int startOffset ) {
            ...
            int tagsCount   = MemoryHelper.ReadInt32( this.Sector.Stream, fileOffset += 0x05 );                           // 0x05(flags)
            int actionCount = MemoryHelper.ReadInt32( this.Sector.Stream, fileOffset += 0x04 + 0x08 * tagsCount + 0x08 ); // 0x04(tagsCount) + tags + 0x08(node_uid)
            fileOffset += 0x04;                                                                                           // 0x04(actionCount)
            for ( var i = 0; i < actionCount; ++i ) {
                int numParamCount    = MemoryHelper.ReadInt32( this.Sector.Stream, fileOffset );
                int stringParamCount = MemoryHelper.ReadInt32( this.Sector.Stream, fileOffset += 0x04 + 0x04 * numParamCount ); // 0x04
