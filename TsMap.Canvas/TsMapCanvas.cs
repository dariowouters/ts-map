    ...
    public partial class TsMapCanvas : Form {
        ...
        private void GenerateTileMap( int         startZoomLevel,
            ...
            var data = new JObject {
                ...
                [ "map_areas_valid" ]         = _mapper.MapAreas.Where( it => it.Valid ).ToList().Count,
                [ "map_areas_hidden" ]        = _mapper.MapAreas.Where( it => it.Hidden ).ToList().Count,
                [ "map_overlays" ]            = _mapper.MapOverlays.Count,
                [ "map_overlays_valid" ]      = _mapper.MapOverlays.Where( it => it.Valid ).ToList().Count,
                [ "map_overlays_hidden" ]     = _mapper.MapOverlays.Where( it => it.Hidden ).ToList().Count,
                [ "map_prefab" ]              = _mapper.Prefabs.Count,
                [ "map_road" ]                = _mapper.Roads.Count,
                [ "map_road_valid" ]          = _mapper.Roads.Where( it => it.Valid ).ToList().Count,
                [ "map_road_hidden" ]         = _mapper.Roads.Where( it => it.Hidden ).ToList().Count,
                [ "map_road_no_look" ]        = _mapper.Roads.Where( it => it.RoadLook == null ).ToList().Count,
                [ "map_road_no_points" ]      = _mapper.Roads.Where( it => it.HasPoints() ).ToList().Count,
                [ "map_triggers" ]            = _mapper.Triggers.Count,
                [ "map_triggers_valid" ]      = _mapper.Triggers.Where( it => it.Valid ).ToList().Count,
                [ "map_triggers_hidden" ]     = _mapper.Triggers.Where( it => it.Hidden ).ToList().Count,
                [ "map_triggers_no_overlay" ] = _mapper.Triggers.Where( it => it.Overlay == null ).ToList().Count,
                ...
                [ "item_fuel" ]               = 0,
                [ "item_garage" ]             = 0,
                [ "item_overlay" ]            = _mapper.MapOverlays.Count,
                [ "item_parking" ]            = 0,
                [ "item_recruitment" ]        = 0,
                [ "item_service" ]            = 0,
                [ "item_train" ]              = 0,
